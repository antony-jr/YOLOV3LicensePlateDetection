# The MIT License
# Copyright (C) 2021, Antony J.R <antonyjr@pm.me>.
# 
# Python3 script to use yolov3 object detection for 
# license plate detection.
# (Optionallly) use tesseract to get the license plate
# number.
#
# This code is written at BigVision LLC. It is based on the OpenCV project. 
# It is subject to the license terms in the LICENSE file found in this distribution 
...
if not os.path.exists("model.weights") or not os.path.isfile("model.weights"):
    # Downlaod the weights.
    print("Downloading Pre-Trained Weights: ")
    wget.download("https://github.com/antony-jr/YOLOV3LicensePlateDetection/releases/download/weights/model.weights")
...
if __name__ == "__main__":
    title = 'License Plate Detection YOLO v3'
    cv.namedWindow(title, cv.WINDOW_NORMAL)

    if (args.image):
        # Open the image file
        if not os.path.isfile(args.image):
            print("Invalid Image File: ", args.image)
            sys.exit(1)
    ...
    if (not args.image):
        vid_writer = cv.VideoWriter("out.avi", cv.VideoWriter_fourcc('M', 'J', 'P', 'G'), 30, (round(
    ...
    while cv.waitKey(1) < 0:
        ...
        # Write the frame if video input
        if not args.image:
            vid_writer.write(frame.astype(np.uint8))

        # Now write the detected frame to program
